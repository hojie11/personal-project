...
class LookAtPoseSampler:
    ...
    def sample(horizontal_mean, vertical_mean, lookat_position, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        ...
        lookat_position = lookat_position.to(device)
        camera_origins = torch.zeros((batch_size, 3), device=device)
...
def batchify_cam_params(device, batch_size : int, fov : float = 18.837):
    c_trg_list = []
    ...
    return torch.stack(c_trg_list).squeeze(1)


if __name__ is '__main__':
    c_trg_list = batchify_cam_params(6)
